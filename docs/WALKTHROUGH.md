...
### 3. Regional Threading & Folia Support
- **Regional Assertions**: `ServerMock` and `PlayerSimulation` now support `assertScheduledOnRegion()` and `assertScheduledOnEntityRegion()`.
- **Task Tracking**: All tasks scheduled via Folia's `RegionScheduler`, `EntityScheduler`, and `GlobalRegionScheduler` are now tracked for later verification.
- **Context Awareness**: `PaperScheduledTask` preserves the original execution context (World, Chunk, Entity), allowing for deep multi-threading verification.

### 4. Binary State & NBT
- **`NbtStateMock`**: High-fidelity Base64 state serialization for entity cloning.
- **Auto-Discovery**: Entities now automatically load default NBT states from `resources-autogenerated/registries/entity_nbt.json` if available.
- **Mapping Logic**: `EntityMock#applyNbt` provides a mapping layer between vanilla NBT tags and internal mock fields (e.g., `CustomName`, `Invulnerable`).
