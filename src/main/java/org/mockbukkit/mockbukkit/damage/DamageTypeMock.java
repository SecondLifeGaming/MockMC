...
public class DamageTypeMock
		implements
			DamageType,
			org.mockbukkit.mockbukkit.generated.org.bukkit.damage.DamageTypeBaseMock
{
	...
	public static DamageTypeMock from(JsonObject data)
	{
		...
		Preconditions.checkArgument(data.has(DAMAGE_SCALING_KEY),
				"JsonObject does not have a field named 'damageScaling'");
		Preconditions.checkArgument(data.has(SOUND_KEY), "JsonObject does not have a field named 'sound'");
		Preconditions.checkArgument(data.has(DEATH_MESSAGE_TYPE_KEY),
	...
	public DamageTypeMock(@NotNull DamageScaling damageScaling, @NotNull DamageEffectMock damageEffect,
			@NotNull NamespacedKey namespacedKey, @NotNull DeathMessageType deathMessageType, float exhaustion,
	 ...
	 *             {@link DamageTypeMock#from(JsonObject)} instead.
	 */
