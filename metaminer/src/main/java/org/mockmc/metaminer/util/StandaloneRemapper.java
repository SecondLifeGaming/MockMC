...
public class StandaloneRemapper
{
	...
	public static File remap(File inputJar, File mappingFile, File outputJar) throws IOException, InterruptedException
	{
		...
		LOGGER.log(Level.INFO, "Remapping {0} to Mojang names via isolated process with prioritized ASM 9.7...",
				inputJar.getName());
		...
		List<String> asmEntries = cpEntries.stream().filter(s -> s.contains("asm-")).toList();
